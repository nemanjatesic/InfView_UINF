...
public class CountDBAction extends AbstractActionI {

	@Override
	public void actionPerformed(ActionEvent arg0) {
		String selected = ((CountState) Frame1.getInstance().getStateManager().getCurrentState()).getSelectedAtribut()
				.getName();
		ArrayList<Boolean> tmp = ((CountState) Frame1.getInstance().getStateManager().getCurrentState())
				.getSelectedButtonsList();
		...
		try {
			ResultSet rs = file.count(selected, groupByList);
			groupByList.add("COUNT");
